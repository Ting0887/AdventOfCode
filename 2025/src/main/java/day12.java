...
public class day12 {
    public static void main(String[] args) throws Exception { 
        File file = new File("src/main/java/testInput/input12.txt"); 
        Scanner sc = new Scanner(file); List<List<int[]>>
        ...
        while (sc.hasNext()) { 
            ...
            // region 開始 
            if (line.contains("x") && line.contains(":")) { readingShapes = false; } 
            if (readingShapes) { 
                if (line.endsWith(":")) { 
    ...
    private static List<int[]> parseShape(List<String> lines) { 
    ...
    private static void normalize(List<int[]> shape) { 
    ...
    private static String encode(List<int[]> shape) { 
    ...
    // ========= Region 判斷 ========= 
    private static boolean canFitRegion( String line, List<List<List<int[]>>> variants, int[] areas ) { 
        ...
        // ========= 回溯 ========= 
        private static boolean dfs( Board board, int[] order, int idx, List<List<List<int[]>>> variants ) { 
        ...
        // ========= Board ========= 
        private static class Board { int W, H; boolean[][] grid; Board(int w, int h) { 
            W = w; 
            H = h; 
            grid = new boolean[h][w]; 
        } 
        private boolean canPlace(List<int[]> shape, int x0, int y0) { 
            for (int[] p : shape) { 
                int x = x0 + p[0]; 
                int y = y0 + p[1]; 
